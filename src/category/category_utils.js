...
class CategoryUtils {
     ...
     * @param {number} level - Category level (0-based)
     * @returns {string} - Cumulative category seed for this level
     */
    static generateCategorySeed(categories, level) {
     ...
     * @param {string} seed - Category seed
     * @returns {number} - Hue value (0-360)
     */
    static generateHueFromSeed(seed) {
     ...
     * @param {number} level - Category level (1-based)
     * @param {Object} options - Styling options
     * @param {string[]} fullCategories - Full category array for seed generation
     ...
     * @param {Object} hierarchy - Category hierarchy from CategoryParser
     * @param {string} namespace - CSS namespace
     * @returns {string} - Hierarchy HTML
     */
    static createHierarchyHTML(hierarchy, namespace = 'fancy-gst') {
        if (!hierarchy || typeof hierarchy !== 'object') {
            return '<div class="no-hierarchy">No categories found</div>';
        }

        const createNode = (node, categoryName, level = 1, parentCategories = []) => {
            ...
            let html = `
                <div class="${namespace}-hierarchy-node" 
                     data-level="${level}" 
                     data-seed="${CoreDOMUtils.escapeHtml(seed)}">
                    <div class="${namespace}-hierarchy-category" data-seed="${CoreDOMUtils.escapeHtml(seed)}">
                        <span class="${namespace}-category-name">${CoreDOMUtils.escapeHtml(categoryName)}</span>
            ...
            if (hasChildren) {
                ...
                Object.entries(node.children).forEach(([childName, childNode]) => {
                    html += createNode(childNode, childName, level + 1, currentCategories);
                });
                html += '</div>';
            }
        ...
        let hierarchyHTML = `<div class="${namespace}-category-hierarchy">`;
        Object.entries(hierarchy).forEach(([categoryName, node]) => {
            hierarchyHTML += createNode(node, categoryName, 1, []);
        });
        hierarchyHTML += '</div>';
    ...
    /**
     * Generate CSS custom properties for categories (seeds only)
     * @param {string[]} categories - List of categories
     * @param {Object} groupMappings - Category group mappings
     * @param {string} namespace - CSS namespace
     * @returns {string} - CSS custom properties with seeds
     */
     ...
     * @param {Object[]} tasks - Array of tasks with categories
     * @returns {Object} - Category statistics
     */
    static getCategoryStatistics(tasks) {
        const stats = {
     ...
     * @param {string[]} availableCategories - Available categories
     * @param {string[]} selectedCategories - Currently selected categories
     * @param {string} namespace - CSS namespace
     * @param {Object} groupSuggestions - Suggested category combinations
