// container/container_ui.ts - Container UI creation and management with toggle visibility
import * as Logger from '@/core/logger';
import { CoreDOMUtils } from '@/core/dom_utils';

Logger.fgtlog('🎨 Container UI loading...');
...
class ContainerUI {
     ...
     */
    async createContainer(): Promise<HTMLElement> {
        // Load CSS first
        ...
        Logger.fgtlog('📦 Main container created');
     ...
     * @returns Table container HTML
     */
    generateTableContainerHTML(): string {
        return `
        ...
        return `
            <div class="${this.namespace}-footer">
                <div class="${this.namespace}-footer-left">
                    ...
                    <span class="${this.namespace}-version">v${process.env.APP_VERSION || 'DEV'}</span>
                    <span class="${this.namespace}-separator">•</span>
     ...
     * @param isEnhancedMode - Whether enhanced mode is active
     */
    updateToggleButton(isEnhancedMode: boolean): void {
     ...
     * @param showCompleted - Whether completed tasks are shown
     */
    updateCompletedToggleButton(showCompleted: boolean): void {
     ...
     */
    showCompletedToggleButton(): void {
    ...
    async loadStyles(): Promise<boolean[]> {
        // Fallback style for container (minimal)
        ...
        const containerCSS = 'css/container.css';
     ...
     * @returns Toggle indicator element
     */
    getToggleIndicator(): HTMLElement | null {
        return this.toggleIndicator;
    }

    /**
     * Get completed toggle indicator element
     * @returns Completed toggle indicator element
     */
    getCompletedToggleIndicator(): HTMLElement | null {
...
Logger.fgtlog('✅ Container UI loaded successfully');