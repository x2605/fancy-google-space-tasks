// manipulator/delete/delete_controller.ts - Task deletion controller
import * as Logger from '@/core/logger';
import { CoreNotificationUtils } from '@/core/notification_utils';
import { OgtFinder } from '@/dom_bringer/finder';
import { OgtDeleteConfirmDialog } from '@/dom_bringer/delete_confirm_dialog';
import { OperationVerifier } from '@/core/operation_verifier';
import { TaskManipulatorBase } from '@/manipulator/common/task_manipulator_base';

Logger.fgtlog('🗑️ Delete Controller loading...');

/**
 * Controller for task deletion with confirmation dialog
 * Handles the complete deletion flow including verification
 */
export class DeleteController extends TaskManipulatorBase {
    ...
    async deleteTask(taskId: string, onComplete: Function | null = null): Promise<void> {
        try {
            ...
            const dialog = await this.waitForElement('div[aria-modal="true"][role="dialog"]', 3000) as HTMLDivElement;
...
Logger.fgtlog('✅ Delete Controller loaded successfully');
